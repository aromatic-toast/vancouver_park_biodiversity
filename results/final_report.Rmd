...
test_parks <- parks[!is.na(parks$park_name), ]
test_parks <- test_parks %>% select(park_name, species_count, species_richness)
parks <- parks %>% 
...
### Outliers
```{r}
knitr::include_graphics("results/outliers.png")
```
...
shinyApp(
  ...
  server = function(input, output) {
    output$table3 <- renderDataTable(head(test_parks, 10)) #  head(parks, 10) head(clipped_gbif,10)
  }
)
```
